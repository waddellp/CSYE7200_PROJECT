...
class ForecasterUtilSpec extends FlatSpec with Matchers {
  private val spark = SparkSession.builder().appName("MultipleLinearRegression").master("local[*]").getOrCreate()
  ...
  it should "work for the Oct2020 test data" in {
    ...
    q.collect.length shouldBe 10113
    source.close()
  }
  ...
  it should "return results only from Oct. 31st" in {
    ...
    qr.collect.length shouldBe 208
    source.close()
  }
  ...
  it should "return results only from October 19th in Alaska" in {
    ...
    qrl.collect().length shouldBe 658
    source.close()
  }
  ...
  it should "return largest earthquake in Alaska during Oct. 2020" in {
    ...
    qrls.collect().toList.head.magnitude.magnitude shouldEqual 7.6
    source.close()
  }
  ...
  it should "return the top 10 hot spots from Oct. 2020" in {
    ...
    t10.head._1 shouldBe "Sand Point"
    t10.head._2.size shouldBe 1262
